...
fn main() -> Result<(), ParseError> {
    setup_logger();
    // unsafe {
    //     let handle = libc::dlopen("./elf".as_ptr() as *const i8, libc::RTLD_NOW);
    //     let f: extern "C" fn(
    //         i64,
    //         i64,
    //         i64,
    ...
    //         f64,
    //         f64,
    //     ) -> f64 = std::mem::transmute(libc::dlsym(handle, "external_func".as_ptr() as *const i8));
    ...
    // let mut loader = elf_loader::Loader::new().with_default_tls_resolver();
    // let exec = loader
    //     .load_exec("/home/slothy/projects/hearth/target/debug/interpreter")
    //     .unwrap();
    //
    // let arch = Arch::current();
    //
    // let writer = DylibWriter::with_config(arch, ElfWriterConfig::default().with_page_size(10000));
    //
    // let relocs = vec![
    ...
    // ];
    //
    // let output = writer.write(&relocs, &symbols).unwrap();

    // panic!(
    //     "{:?} {:?} {:?}",
    //     output.base_addr, output.data_vaddr, output.text_vaddr,
    // );
    // let loader = Loader::new()
    //     .with_default_tls_resolver()
    //     .load_exec(ElfBinary::new("kirk", &output.data))
    //     .unwrap();
    //
    // info!(?loader);
    // let entry = loader.entry();
    // let entry = loader.relocator().relocate().unwrap().entry();
    //
    // for d in &output.data[entry..] {
    //     info!(?d);
    // }
...
fn run_dynamic_linking(is_lazy: bool) {
    ...
    std::fs::write("helper", &helper_output.data).unwrap();
    ...
    panic!("{symbol_map:?} {:?}", symbol_lookup("external_func"));
    ...
    let relocator = dylib
        ...
        .scope(std::slice::from_ref(&helper_relocated))
        .lazy(is_lazy);
